...
const OrderDetails = (props) => {
    ...
    const submitRatingReview = async (event) => {
        event.preventDefault();
        try {
            // Loop through order items to find the relevant item
            order.orderItems.forEach(async (item) => {
                
                const rateProduct = await axios.get(`http://localhost:5000/api/products/${item.product}`);
               
                const response = await axios.put(`http://localhost:5000/api/products/rating/${item.product}`, {
                    rating: rating,
                    numRev: rateProduct.data.numRev + 1,
                });
    ...
    :(
        <div>
            ...
            <button id="bthb"><a href='http://localhost:3000' id="bth">Back to Home</a></button>
            ...
            <div className="row-container">
                                    ...
                                    order.orderItems.map((item)=>(
                                        <li key={item.product} className="order-item">
                                            <div className="row1 order-row1">
                                                ...
                                                <div className="small">
                                                    <Link to={`/products/product/${item.product}`}>{item.name}</Link><br />
                                                    <a className="add-to-cart" href={`/pdfview/product/${item.product}`}>Preview as PDF</a>
                                                </div>
                ...
                <div className="col-7">
                    ...
                    <br/>
                    <div className="card-body">
                    <form onSubmit={submitRatingReview}>
                        ...
                        <button className="add-to-cart" type="submit">Submit</button><br/>
                        {alertMessage && (
